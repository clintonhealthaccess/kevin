...
//		}
			...
			if(surveyQuestion.getType().equals(QuestionType.TABLE)){
				...
				for(int r = 0; r < surveyTableRows.size(); r++){
					for(int c = 0; c < surveyTableColumns.size(); c++){						
						...
						String surveyQuestionRow = LanguageUtils.getText(surveyTableRows.get(r).getNames());						
						String surveyQuestionColumn = LanguageUtils.getText(surveyTableColumns.get(c).getNames());											
	...
	SurveyExportDataPoint getBasicInfoSurveyExportDataPoint(Organisation facility, Survey survey, SurveyObjective surveyObjective, 
		...
		dataPoint.add(getSurveyExportDataItem(SURVEY_HEADER, LanguageUtils.getText(survey.getNames()), dataPoint.size()));
		dataPoint.add(getSurveyExportDataItem(OBJECTIVE_HEADER, LanguageUtils.getText(surveyObjective.getNames()), dataPoint.size()));
		dataPoint.add(getSurveyExportDataItem(SECTION_HEADER, LanguageUtils.getText(surveySection.getNames()), dataPoint.size()));		
		...
		dataPoint.add(getSurveyExportDataItem(QUESTION_HEADER, LanguageUtils.getText(surveyQuestion.getNames()), dataPoint.size()));
	...
	SurveyExportDataPoint getSimpleSurveyExportDataPoint(SurveyExportDataPoint simpleDataPoint, Organisation facility, SurveyQuestion surveyQuestion, SurveyElement surveyElement, List<Object> surveyQuestionDataItems){
		...
		simpleDataPoint.add(getSurveyExportDataItem(DATA_NAME_HEADER, LanguageUtils.getText(dataElement.getNames()), simpleDataPoint.size()));
