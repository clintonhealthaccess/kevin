...
public class Initializer {
	...
	static def createDashboard() {
		if (!DashboardProgram.count()) {
			...
			def calculation1 = new Summ(expression:"\$"+NormalizedDataElement.findByCode("Constant 10").id, code:"Ratio constant 10", timestamp:new Date())
			...
			def calculation2 = new Summ(expression:"\$"+NormalizedDataElement.findByCode("Constant 20").id, code:"Ratio constant 20", timestamp:new Date())
			...
			def calculation3 = new Summ(expression:"\$"+NormalizedDataElement.findByCode("Element 1").id, code:"Ratio 1", timestamp:new Date())
			...
			def calculation4 = new Summ(expression:"\$"+NormalizedDataElement.findByCode("Element 2").id, code:"Ratio 2", timestamp:new Date())
			...
			def calculation5 = new Summ(expression:"\$"+NormalizedDataElement.findByCode("Element 3").id, code:"Ratio 3", timestamp:new Date())
			...
			def calculation6 = new Summ(expression:"\$"+NormalizedDataElement.findByCode("Element 3").id, code:"Ratio 4", timestamp:new Date())
	...
	static def createDsr() {
		if (!DsrTarget.count()) {
			...
			def dsrAvg = new Summ(expression:"\$"+NormalizedDataElement.findByCode("Constant 10").id, code:"Dsr Average constant 10", timestamp:new Date())
			dsrAvg.save(failOnError: true)
			
			def dsrSum = new Summ(expression: "\$"+NormalizedDataElement.findByCode("Constant 10").id, code:"Dsr Sum constant 10", timestamp:new Date());
	...
	static def createFct() {
		if (!FctTarget.count()) {
			...
			def sumZero = new Summ(expression: "\$"+NormalizedDataElement.findByCode("ZERO").id, code:"Sum ZERO", timestamp:new Date());
			sumZero.save(failOnError: true);
			
			def sumOne = new Summ(expression: "\$"+NormalizedDataElement.findByCode("ONE").id, code:"Sum ONE", timestamp:new Date());
			sumOne.save(failOnError: true);
			
			def sumMixHC = new Summ(expression: "\$"+NormalizedDataElement.findByCode("MixHC").id, code:"Sum MixHC", timestamp:new Date());
			sumMixHC.save(failOnError: true);
				
			def sumMixDH = new Summ(expression: "\$"+NormalizedDataElement.findByCode("MixDH").id, code:"Sum MixDH", timestamp:new Date());
	...
	static def createCalculationExport(){
		if(!CalculationExport.count()){
			...
			def dEtwo = Summ.findByCode("Ratio constant 20");
			def dEthree = Summ.findByCode("Ratio constant 10");
			def dEfour = Summ.findByCode("Ratio 1");
			def dEfive = Summ.findByCode("Ratio 2");
