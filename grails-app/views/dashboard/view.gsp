			...
			<div id="top" class="main">
		    	...
		    	<ul class="form-heading-list horizontal inline-list">
		    		<g:each in="${dashboard.objectivePath}" var="objective">
			    		<li>
			    			<g:link controller="dashboard" action="view" params="[period: currentPeriod.id, entity: objective.id, organisation: currentOrganisation.id]"><g:i18n field="${objective.names}"/></g:link>
			    		</li>
		    		</g:each>
		    		<li>
			    	...
			    	<div class="left">
				    	...
				    	<ul class="horizontal">
				    		<g:each in="${dashboard.organisationPath}" var="organisation">
					    		<li>
					    			<g:link controller="dashboard" action="view" params="[period: currentPeriod.id, entity: entity.id, organisation: organisation.id]">${organisation.name}</g:link>
					    		</li>
				    		</g:each>
				    		<li>
		    		...
		    		<div id="values">
						    			...
						    			<th class="cell label top col-${dashboardEntity.id}" data-col="${dashboardEntity.id}">
						    				...
						    				<g:if test="${!dashboardEntity.isTarget()}">
												<a class="cluetip" 
													...
													href="${createLink(controller:'dashboard', action:'view', params:[period: currentPeriod.id, entity: dashboardEntity.id, organisation: currentOrganisation.id])}"
										...
										<td class="highlighted value cell row-${organisation.id} col-${dashboardEntity.id}" data-row="${organisation.id}" data-col="${dashboardEntity.id}">
											<g:if test="${percentage!=null}">
												<div style="background-color: ${percentage.color};">
												    <span>
												    	<a class="no-link" href="${createLink(controller:'dashboard', action:'explain', params:[entity: dashboardEntity.id, organisation: organisation.id, period: currentPeriod.id])}">
			    		...
			    		<shiro:hasPermission permission="admin:dashboard">
			    			<div class="float-right">
								<div><a class="add-row" href="${createLinkWithTargetURI(controller:'dashboardTarget', action:'create', params:[entity: entity.id])}"><g:message code="default.add.label" args="[message(code:'dashboard.target.label')]" default="Add target"/></a></div>
								<div><a class="add-row" href="${createLinkWithTargetURI(controller:'dashboardObjective', action:'create', params:[entity: entity.id])}"><g:message code="default.add.label" args="[message(code:'dashboard.objective.label')]" default="Add objective"/></a></div>
							</div>
						</shiro:hasPermission>
				    	<!-- ADMIN SECTION END -->
