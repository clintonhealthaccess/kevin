<div id="add-iteration" class="entity-form-container togglable">
	...
	<g:form url="[controller:'iteration', action:'save']" useToken="true">
		...
		<g:datePicker name="startDate" value="${iteration.startDate}"  precision="day"  years="${Calendar.getInstance().get(Calendar.YEAR)..1990}"/></span>
		<span class="display-in-block"><g:message code="general.text.enddate" default="End Date" />
		<g:datePicker name="endDate" value="${iteration.endDate}" precision="day"  years="${Calendar.getInstance().get(Calendar.YEAR)..1990}"/></span>
