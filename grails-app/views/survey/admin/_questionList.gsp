					...
					<div class="hidden dropdown-list">
						<ul>
							<li><a id="add-simplequestion-link" class="flow-add"
								href="${createLink(controller:'simpleQuestion', action:'create', params:[subObjectiveId: subobjective.id])}">New
									Simple Question</a>
							</li>
							<li><a id="add-checkboxquestion-link" class="flow-add"
								href="${createLink(controller:'checkboxQuestion', action:'create', params:[subObjectiveId: subobjective.id])}">New
									Checkbox Question</a>
							</li>
							<li><a id="add-tablequestion-link" class="flow-add"
								href="${createLink(controller:'tableQuestion', action:'create', params:[subObjectiveId: subobjective.id])}">New
									Table Question</a>
							</li>
						</ul>
					</div>
					...
					<g:each in="${questions}" status="i" var="question">
						<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
							<td><a class="question-explainer display-in-block"
								...
								onclick="return false;"> ${question.getString(i18n(field:
									question.names).toString(),100)} </a></td>
