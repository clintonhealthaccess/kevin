...
<g:if test="${!invalidQuestions.isEmpty()}">
	<div class="rounded-box-top"><g:message code="survey.followingquestionnotpassvalidation.label" default="The following questions do not pass validation, please check" />:</div>
	<form id="survey-form">
		<g:each in="${invalidQuestions}" var="question" status="i">
			<div class="invalid-question ${i!=0?'hidden':''} rounded-box-bottom">
				...
				<g:if test="${i!=0}">
					<a href="#" onclick="$(this).parents('.invalid-question').hide();$(this).parents('.invalid-question').prev().show();"><g:message code="general.text.previous" default="Previous" /></a>
				</g:if>
				<g:if test="${i!=invalidQuestions.size()-1}">
					<a href="#" onclick="$(this).parents('.invalid-question').hide();$(this).parents('.invalid-question').next().show();"><g:message code="general.text.next" default="Next" /></a>
				</g:if>
			</div>
		</g:each>
	</form>
