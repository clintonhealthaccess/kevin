...
<g:set var="question" value="${surveyElement.surveyQuestion}"/>
	<li data-code="${surveyElement.id}" id="survey-element-${surveyElement.id}">
		<a	class="no-link cluetip" onclick="return false;" title="${i18n(field:surveyElement.dataElement.names)}"
			href="${createLink(controller:'surveyElement', action:'getSurveyElementExplainer', params:[surveyElement: surveyElement.id])}"
			rel="${createLink(controller:'surveyElement', action:'getSurveyElementExplainer', params:[surveyElement: surveyElement.id])}">
