<g:each in="${constants}" var="constant">
	<li data-code="c${constant.id}" id="constant-${constant.id}">
		<a class="no-link cluetip" onclick="return false;" title="${i18n(field: constant.names)}"
			href="${createLink(controller:'constant', action:'getConstantDescription', params:[constant: constant.id])}"
			rel="${createLink(controller:'constant', action:'getConstantDescription', params:[constant: constant.id])}">
