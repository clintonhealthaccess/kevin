...
<title><g:message code="dsrTable.view.label" default="District Summary Reports" />
</title>
...
<r:require modules="fliptext,cluetip,dropdown,nicetable,report" />
</head>
<body>
	<div id="dsr">
		<div id="top" class="box">
			<div class="filter">
				...
				<div class="dropdown white-dropdown">
					<a class="selected" href="#" data-period="${dsrTable.period.id}" data-type="period"><g:dateFormat format="yyyy" date="${dsrTable.period.startDate}" />
					</a>
					<div class="hidden dropdown-list">
						<ul>
							<g:each in="${periods}" var="period">
								<li><a href="${createLink(controller: " dsr", action:"view", params:[period:period.id, objective: dsrTable.objective?.id, organisation:dsrTable.organisation?.id])}"> <span><g:dateFormat format="yyyy" date="${period.startDate}" /> </span> </a>
								</li>
							</g:each>
						</ul>
					</div>
			...
			<div class="filter">
				...
				<div class="dropdown white-dropdown">
					...
					<div class="hidden dropdown-list">
						<ul>
							<g:render template="/templates/organisationTree" model="[controller: 'dsr', action: 'view', organisation: organisationTree, current: dsrTable.organisation, params:[period: periods.startDate, objective: dsrTable.objective?.id], displayLinkUntil: displayLinkUntil]" />
						</ul>
					</div>
				</div>
			</div>
			<div class="filter">
				...
				<div class="dropdown white-dropdown">
					<g:if test="${dsrTable.objective != null}">
						<a class="selected" href="#" data-organisation="${dsrTable.objective.id}" data-type="objective"><g:i18n field="${dsrTable.objective.names}" />
						</a>
					</g:if>
					<g:else>
						<a href="#" class="selected" data-type="objective"> Select Strategic Objective</a>
					</g:else>
					<div class="hidden dropdown-list">
						<g:if test="${!objectives.empty}">
							<ul>
								<g:each in="${objectives}" var="objective">
									<li><span> <a href="${createLink(controller: " dsr", action:"view", params:[period: dsrTable.period.id, objective: objective?.id, organisation:dsrTable.organisation?.id])}"> <g:i18n field="${objective.names}" /> </a> </span> <shiro:hasPermission permission="admin:dsr">
											<span> <g:link controller="dsrObjective" action="edit" id="${objective.id}" class="flow-edit">(Edit)</g:link> </span>
											<span> <g:link controller="dsrObjective" action="delete" id="${objective.id}" class="flow-delete">(Delete)</g:link> </span>
