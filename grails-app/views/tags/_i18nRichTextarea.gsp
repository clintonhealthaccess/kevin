...
<div class="row ${hasErrors(bean:bean,field:field, 'errors')}">
	<g:each in="${locales}" var="locale" status="i">
		...
		<div class="toggle-entry ${i!=0?'hidden':''}" data-toggle="${locale}">
			...
			<textarea id="area-${random}-${locale}" type="${type}" class="idle-field" name="${name}.${locale}" style="width: ${width}px; height: ${height}px;" id="richtext-${locale}" class="rich-text-area" >${value[locale]}</textarea>
		</div>
	</g:each>
