<div id="add-enum" class="entity-form-container togglable">
	...
	<g:form url="[controller:'enum', action:'save']" useToken="true">
		<g:i18nInput name="names" bean="${enumeration}"
			value="${enumeration?.names}" label="Name" field="names" />
			<g:i18nRichTextarea name="descriptions" bean="${enumeration}" value="${enumeration.descriptions}" label="Descriptions" field="descriptions" height="150"  width="300" maxHeight="150" />
		<g:input name="code" label="Code" bean="${enumeration}" field="code" />
		<div id="option-block" class="admin-table-list">
			<table id="enum-option">
				<g:each in="${enumeration.enumOptions}" status="i" var="option">
					<tr class="enum-option">
						<td id="enum-option-${option.id}"><g:render
								template="/templates/enumOption" model="[option: option]" />
						</td>
					</tr>
				</g:each>
			</table>
			<g:if test="${enumeration.id != null}">
				<div>
					<a class="add-option-link flow-add float-right"
						href="${createLink(controller:'enumOption', action:'create',params:[enumId: enumeration.id])}">
					...
					$('#add-enum').flow({
						...
						onSuccess : function(data) {
							if (data.result == 'success') {
								...
								var checkboxOptionId = data.newEntity;
								$('#enum-option').append(
										'<tr class="enum-option"><td id="enum-option-'+checkboxOptionId.id+'">'
