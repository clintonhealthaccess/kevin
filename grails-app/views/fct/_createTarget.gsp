<div id="add-dsr-target" class="entity-form-container togglable">
	...
	<g:form url="[controller:'fctTarget', action:'save']" useToken="true">
		...
		<div class="row">
			<div>
				<a id="new-dsr-objective-link" class="float-right"  href="${createLink(controller:'fctObjective', action:'create')}">New FCT Objective</a>
			</div>
			<div class="clear"></div>
		...
		$('#add-dsr-target').flow({
			...
			onSuccess: function(data) {
				if (data.result == 'success') {
					var fctObjective = data.newEntity;
					$('.objective-list').append('<option value="'+fctObjective.id+'">'+fctObjective.names[data.locale]+'</option>');
