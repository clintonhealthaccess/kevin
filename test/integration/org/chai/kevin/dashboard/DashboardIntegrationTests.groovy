...
abstract class DashboardIntegrationTests extends IntegrationTests {
	...
	def newDashboardObjective(def code, def objective) {
		return new DashboardObjective(code: code, objective: objective, weight: 1).save(failOnError: true)
	}
	...
	def newDashboardTarget(def code, def calculation, def parent, def weight) {
		def dashboardTarget = new DashboardTarget(code: code, calculation: calculation, objective: parent, weight: weight).save(failOnError: true)	
