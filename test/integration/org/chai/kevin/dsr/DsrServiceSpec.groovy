...
class DsrServiceSpec extends DsrIntegrationTests {
	...
	def "test normal dsr service"() {
		...
		when:
		dsrTable = dsrService.getDsrTable(organisation, objective, period, new HashSet([DISTRICT_HOSPITAL_GROUP, HEALTH_CENTER_GROUP]))
		...
		when:
		dsrTable = dsrService.getDsrTable(organisation, objective, period, new HashSet([DISTRICT_HOSPITAL_GROUP]))
