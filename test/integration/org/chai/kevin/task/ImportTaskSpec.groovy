...
class ImportTaskSpec extends IntegrationTests {
	...
	def "test constraints"() {
		...
		when:
		new NominativeImportTask(rawDataElementId: 1, periodId: 1, user: user, status: TaskStatus.NEW, delimiter: ',', inputFilename: 'test.csv').save(failOnError: true)
		
		then:
		thrown ValidationException
		
		when:
		new NominativeImportTask(rawDataElementId: 1, periodId: 1, user: user, status: TaskStatus.NEW, encoding: 'UTF-8', inputFilename: 'test.csv').save(failOnError: true)
		
		then:
		thrown ValidationException
		
		when:
		new NominativeImportTask(rawDataElementId: 1, periodId: 1, user: user, status: TaskStatus.NEW, encoding: 'UTF-8', delimiter: ',').save(failOnError: true)
		
		then:
		thrown ValidationException
		
		when:
		new NominativeImportTask(rawDataElementId: 1, periodId: 1, user: user, status: TaskStatus.NEW, encoding: 'UTF-8', delimiter: ',', inputFilename: 'test.pdf').save(failOnError: true)
	...
	def "check the type of files uploaded"() {
		expect:
		ImportTask.getFileType('nominativeTestFile.csv.zip') == FileType.ZIP
		ImportTask.getFileType('nominativeTestFile.csv') == FileType.CSV
